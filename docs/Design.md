---
version: alpha
name: "React Project Accelerator - Dual Theme Design System"
description: "Supports light and dark themes via Tailwind CSS class-based dark mode. All tokens are CSS custom properties in src/index.css using HSL. Dark mode activates via .dark class on the root element, toggled through ThemeContext."
dark-mode-strategy: "class" # tailwind.config.ts: darkMode: ['class']
...
Dual-theme design system for React Project Accelerator. All color tokens are CSS custom properties (`hsl(...)`) defined in `src/index.css`. Dark mode is class-based — Tailwind applies `.dark` to the root element via `ThemeContext`, toggled by `ThemeToggle` in the nav.
...
All tokens follow the pattern `--token-name: H S% L%` (no commas). Referenced in Tailwind as `hsl(var(--token-name))` or with opacity as `hsl(var(--token-name) / 0.5)`.

```css
/* Light — :root */          /* Dark — .dark */
--background: 0 0% 100%;     --background: 0 0% 9%;
--foreground: 0 0% 5%;       --foreground: 0 0% 98%;
--card:       0 0% 100%;     --card:       0 0% 15%;
--border:     220 13% 91%;   --border:     0 0% 25%;
--muted:      210 20% 98%;   --muted:      0 0% 12%;
--primary:    217 91% 60%;   --primary:    217 91% 60%; /* unchanged */
```
...
All tokens are defined in `src/index.css` as HSL CSS custom properties. Edit there — **not** in `tailwind.config.ts` — to change theme colors. Tailwind semantic color names (`bg-background`, `text-foreground`, `border-border`, etc.) map to these tokens automatically.
...
### Gray Palette (Tailwind Override)

Both `gray` and `slate` are overridden in `tailwind.config.ts` to use a neutral true-gray scale (not the default blue-tinted slate):
...
| Edit `src/index.css` `:root` and `.dark` blocks to change theme colors globally | Don't change colors in `tailwind.config.ts` — the semantic tokens live in `index.css` |
| Use [tweakcn.com](https://tweakcn.com/) to preview and generate shadcn/ui theme variations | Don't invent new CSS variables without adding them to both `:root` and `.dark` |
